...
*Bounding Box 2D Unpack Patch*
The Bounding Box 2D Unpack patch takes the 2D coordinates generated by the Hand Bounding Box patch and unpacks them into individual outputs. The Bounding Box 2D Unpack patch can be used together with the Hand Finder, Hand Select and Hand Bounding Box patches to create hand tracking effects.
	bounding box => top/bottom left/right + center + width + height
...
Use the Counter patch to count pulses. You can then make something happen when each pulse is counted or when a specific number of pulses are counted.
	increase + decrease + jump + jump to number, maximum counter => count

*Comment Patch*
Use the Comment patch to comment on one or more patches.
