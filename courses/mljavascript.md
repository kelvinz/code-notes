...
```js
	...
	function loadCSV( filename, {	converters = {},
		...
		if ( splitTest ) {
			//	if split test is a number
			//	use that number
			//	else it means it's the value true aka default value
			//	where we will split the data into 2, 50 : 50
			const trainSize = _.isNumber( splitTest ) ? splitTest : Math.floor( data.length / 2 )

			return {
				//	everything from 0 up to trainSize but not including trainSize
				features: data.slice( 0, trainSize ),
				labels: labels.slice( 0, trainSize ),
				//	take everything from trainSize to end of array including trainSize
				testFeatures: data.slice( trainSize  ),
				testLabels: data.slice( trainSize )
			}
		} else {
			return { features: data, labels: labels }
			//	optional to use es6 shorten labels: labels to just labels
			//	since name & value is same
			//	aka
